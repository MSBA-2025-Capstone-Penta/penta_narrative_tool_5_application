...
# def _cache_dir(out_dir: Path, model_name: str) -> Path:
#     d = out_dir / f"emb_cache_{model_name.replace('/', '_')}"
#     d.mkdir(parents=True, exist_ok=True)
#     return d
...
# def _spacy_extract_batch(texts):
#     """Run spaCy PERSON NER in batches; fallback to CPU single-proc on failure."""
#     if not HAS_SPACY:
#         return [extract_persons_regex(t) for t in texts]
#     nproc = 1 if SPACY_ON_GPU else SPACY_NPROC
#     try:
#         docs = nlp.pipe(texts, batch_size=SPACY_BATCH_SIZE, n_process=nproc)
#         return _extract_from_docs(docs)
#     except Exception as e:
#         try:
#             print(
#                 f"[names] spaCy pipe failed ({type(e).__name__}: {e}). Retrying on CPU, n_process=1...")
#             try:
#                 spacy.require_cpu()
#             except Exception:
#                 pass
#             model_name = getattr(nlp, "meta", {}).get("name", None)
#             nlp_cpu = spacy.load(model_name or "en_core_web_sm",
#                                  disable=["tagger", "lemmatizer", "textcat", "morphologizer", "attribute_ruler"])
#             docs = nlp_cpu.pipe(texts, batch_size=min(
#                 128, SPACY_BATCH_SIZE), n_process=1)
#             return _extract_from_docs(docs)
#         except Exception as e2:
#             print(
#                 f"[names] spaCy CPU retry also failed ({type(e2).__name__}: {e2}). Falling back to regex.")
#             return [extract_persons_regex(t) for t in texts]
